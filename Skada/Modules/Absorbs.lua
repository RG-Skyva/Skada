...
Skada:RegisterModule("Absorbs", function(L, P, G)
	...
	local band, tsort, tinsert, tremove, max = bit.band, table.sort, table.insert, table.remove, math.max
	...
	local ABSORB_SPELLS = {
		...
		[67222] = {school = 0x01, dur = 86400, cap = 1000000}, -- Twin Val'kyr: Light Essence (variant)
		[67223] = {school = 0x01, dur = 86400, cap = 1000000}, -- Twin Val'kyr: Light Essence (variant)
		[67224] = {school = 0x01, dur = 86400, cap = 1000000}, -- Twin Val'kyr: Light Essence (variant)
		[65684] = {school = 0x01, dur = 86400, cap = 1000000}, -- Twin Val'kyr: Dark Essence
		[67176] = {school = 0x01, dur = 86400, cap = 1000000}, -- Twin Val'kyr: Dark Essence (variant)
		[67177] = {school = 0x01, dur = 86400, cap = 1000000}, -- Twin Val'kyr: Dark Essence (variant)
		[67178] = {school = 0x01, dur = 86400, cap = 1000000} -- Twin Val'kyr: Dark Essence (variant)
	}

	local LIGHT_ESSENCES = {[65686] = true, [67222] = true, [67223] = true, [67224] = true}
	local DARK_ESSENCES = {[65684] = true, [67176] = true, [67177] = true, [67178] = true}
	local LIGHT_ESSENCE_DAMAGE = {
		-- Unleashed Light (orbs)
		[65795] = true, [67238] = true, [67239] = true, [67240] = true,
		-- Light Vortex
		[66048] = true, [67203] = true, [67204] = true, [67205] = true,
		-- Light Surge
		[65767] = true, [67274] = true, [67275] = true, [67276] = true
	}
	local DARK_ESSENCE_DAMAGE = {
		-- Unleashed Dark (orbs)
		[65808] = true, [67172] = true, [67173] = true, [67174] = true,
		-- Dark Vortex
		[66059] = true, [67155] = true, [67156] = true, [67157] = true,
		-- Dark Surge
		[65769] = true, [67265] = true, [67266] = true, [67267] = true
	}
	...
	local valanyr_wielders = {} -- players with Blessing of Ancient Kings (64411)
	...
	local function handle_shield(t, points)
		...
		elseif shield_id == 64413 then -- Protection of Ancient Kings (Vala'nyr)
			-- The local player's heal and aura refresh share a timestamp. For other
			-- players the aura event can arrive later. A queue is required because
			-- several HoTs can heal before their aura refresh events arrive. Each aura
			-- event consumes exactly one matching heal.
			local heal = heals[dstName] and heals[dstName][srcName]
			local queue = heal and heal.valanyr
			if queue then
				local max_delay = (t.srcGUID and t.srcGUID == UnitGUID("player")) and 0 or VALANYR_REMOTE_DELAY
				for i = 1, #queue do
					local pending = queue[i]
					local delay = pending and (t.timestamp - pending.ts)
					local same_source = not t.srcGUID or not pending.srcGUID or t.srcGUID == pending.srcGUID
					local same_target = not t.dstGUID or not pending.dstGUID or t.dstGUID == pending.dstGUID

					if delay and delay >= 0 and delay <= max_delay and same_source and same_target then
						pending = tremove(queue, i)
						amount = min(SHIELD.cap, amount + floor((pending.amount or 0) * 0.15))
						del(pending)
						break
					end
				end
		...
		if LIGHT_ESSENCE_DAMAGE[t.spellid] or DARK_ESSENCE_DAMAGE[t.spellid] then
			...
			if essence_ids then
				for essence_id in pairs(essence_ids) do
					local sources = shields[dstName][essence_id]
					if sources then
						for _, essence in pairs(sources) do
							if essence.ts > timestamp then
		...
		for spellid, sources in pairs(shields[dstName]) do
			for srcName, spell in pairs(sources) do
				if spell.ts > timestamp then
					-- Encounter essences identify matching encounter damage only.
					-- They are never valid sources for the player's absorb meter.
					if LIGHT_ESSENCES[spellid] or DARK_ESSENCES[spellid] then
						-- nothing
	...
	local function spell_heal(t)
		...
		-- Only heals performed while Blessing of Ancient Kings is active can create
		-- Protection of Ancient Kings. Keep them in FIFO order per source and target
		-- so delayed remote aura events cannot overwrite simultaneous HoT ticks.
		local wielder_key = t.srcGUID or t.srcName
		if wielder_key and valanyr_wielders[wielder_key] then
			local queue = heal.valanyr or new()
			heal.valanyr = queue
			for i = #queue, 1, -1 do
				if t.timestamp - queue[i].ts > VALANYR_REMOTE_DELAY then
					del(tremove(queue, i))
				end
			end
			local pending = new()
			pending.ts = t.timestamp
			pending.amount = t.amount
			pending.srcGUID = t.srcGUID
			pending.dstGUID = t.dstGUID
			tinsert(queue, pending)
		end

		-- Divine Aegis still uses the most recent heal stored on the parent entry.
		heal.ts = t.timestamp
		heal.amount = t.amount
		heal.srcGUID = t.srcGUID
		heal.srcName = t.srcName
		heal.dstGUID = t.dstGUID
		heal.dstName = dstName
	end

	local function valanyr_blessing(t)
		if t.spellid ~= VALANYR_BLESSING then return end

		-- 64411 is carried by the Val'anyr owner. Prefer the aura destination because
		-- its source can be inconsistent on some 3.3.5 servers.
		local wielder_key = t.dstGUID or t.dstName or t.srcGUID or t.srcName
		if not wielder_key then return end

		if t.event == "SPELL_AURA_REMOVED" then
			valanyr_wielders[wielder_key] = nil
		else
			valanyr_wielders[wielder_key] = true
		end
	end
